...
def ask_custom_parameters(config):
    ...
    print("Job counts left blank (shown as [None]) are figured out automatically")
    print("from the simulation period and each source's real historical rate.")
    ...
    config.simulation_period_hours = ask_for_int("Simulation period (hours)", config.simulation_period_hours)
    config.job_limit = ask_for_int("Customer RMA jobs (blank = auto)", config.job_limit)
    config.advex_job_count = ask_for_int("AdvEx jobs (blank = auto)", config.advex_job_count)
    config.reship_job_count = ask_for_int("Reship jobs (blank = auto)", config.reship_job_count)
    config.production_job_count = ask_for_int("Production jobs (blank = auto)", config.production_job_count)
